...
export class ButtonComponent {
    title: string = 'button';
    description: string = 'A button is a clickable element that triggers an action or event. It supports multiple styles (fill, outline, text, etc.), colors (primary, secondary, success, etc.), and can be disabled or have full width.';

    public responsive = inject(ResponsiveService);
    screen = computed(() => this.responsive.breakpoint());

    models: MozButtonModel[] = ['fill', 'flavor', 'outline', 'text', 'tonal', 'elevated', 'glass'];
    colors: MozButtonColor[] = ['primary', 'secondary', 'success', 'warn', 'danger'];
